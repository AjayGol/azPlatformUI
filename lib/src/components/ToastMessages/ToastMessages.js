...
const ToastMessages = forwardRef(({ closeIconLib, closeIconName, closeIconSize, severityIconLib, severityIconName, severityIconSize, position = 'top-right', sticky = false, severity = 'success', life, style, contentStyle, closable, summary, detail, visible, content, }, ref) => {
    ...
    const showToast = newToastProps => {
        const newToast = {
            ...
            closable: newToastProps.closable !== undefined
                ? newToastProps.closable
                : closable,
            position: newToastProps.position || position,
            sticky: newToastProps.sticky !== undefined ? newToastProps.sticky : sticky,
            severity: newToastProps.severity || severity,
            life: newToastProps.life || life,
            visible: newToastProps.visible !== undefined ? newToastProps.visible : visible,
    ...
    return (React.createElement(React.Fragment, null, toasts.map((toast, index) => (React.createElement(Toast, Object.assign({ key: toast.id, style: style }, toast, { onClose: () => {
