...
describe('TabsBar', () => {
  ...
  test('renders correctly with default props', () => {
    const {getByText} = render(
      <TabsBar links={tabs} selectedLink={tabs[0]} onClick={onClickMock} />,
    );

    // Ensure that all tabs are rendered
  ...
  test('shows the correct active tab styling', () => {
    const {getByText} = render(
      <TabsBar links={tabs} selectedLink={tabs[1]} onClick={onClickMock} />,
    );
  ...
  test('updates selected tab when a new tab is clicked', () => {
    ...
    // Ensure Tab 2 is active and has the primary color border
