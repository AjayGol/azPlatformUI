...
const FileViewer = ({
          ...
          <TouchableOpacity
            ...
            style={[
              styles.paginationDot,
              {
                ...
                backgroundColor:
                  index === swipeDot ? colors.textColor : colors.white,
              },
            ]}
          />
        ))}
  ...
  const onPageScroll = (page: number, numberOfPages: number) => {
  ...
  return (
    <View ref={containerRef} onLayout={handleLayout} style={styles.container}>
      <View style={styles.header}>
        <View style={{...styles.textView, marginRight: 10}}>
          <TextInput
        ...
        <Text style={styles.headerText}>of {totalPages}</Text>
        <View style={styles.zoomControls}>
          <Icon
            ...
            size={20}
          />
          <View style={{...styles.textView, marginHorizontal: 5}}>
            <TextInput
              style={styles.zoomInput}
              value={zoomInputValue}
              onChangeText={handleZoomChange}
              keyboardType="numeric"
            />
      ...
      <View style={[styles.pdfContainer, containerStyle]}>
        <Pdf
          ...
          onPageChanged={onPageScroll}
          onError={onError}
          showsHorizontalScrollIndicator={false}
          onLoadComplete={onLoadComplete}
          style={[styles.pdf, containerStyle]}
        />
      </View>
      {isSwiperVisible && (
        <>
          <View style={{...styles.swiperContainer, ...containerStyle}}>
            <View style={styles.swiperButtons}>
              <TouchableOpacity onPress={handleClose}>
                <Text style={styles.buttonTextStyle}>Close</Text>
              </TouchableOpacity>
              <TouchableOpacity onPress={presentationClick}>
            ...
            <Carousel
              ref={carouselRef}
              loop={false}
              style={{height: swiperHeight * 0.22}}
              renderItem={({index}) => {
                const itemsCount = Math.min(5, totalPages - index * 5); // Ensure we don't have more than 5 items per slide
                const justifyContentStyle =
                  itemsCount <= 2 ? 'flex-start' : 'space-between';
                const itemMargin = itemsCount <= 2 ? 8 : 0; // More margin when only 2 items

                return (
                  <View
                    key={index}
                    style={{
                      ...styles.swiperSlideContainer,
                      justifyContent: justifyContentStyle,
                      height: swiperHeight * 0.22,
                    }}>
                      ...
                      if (pageNumber <= totalPages) {
                        return (
                          <TouchableOpacity
                            ...
                            style={{
                              ...styles.swiperSlide,
                              width: containerWidth / 5 - 20,
                              height: '100%',
                              marginHorizontal: itemMargin,
                              borderWidth: pageNumber === currPage ? 2 : 0,
                            }}
                            onPress={() => {
                            ...
                            <View style={styles.slideIndexOverlay}>
                              <Text style={styles.textStyle}>{pageNumber}</Text>
                            </View>
                            <View style={styles.swiperPdf}>
                              <Pdf
                                source={{uri: url, cache: true}}
                                page={pageNumber}
                                trustAllCerts={false}
              ...
              height={swiperHeight * 0.2}
              autoPlay={false}
              data={Array.from({length: Math.ceil(totalPages / 5)})}
